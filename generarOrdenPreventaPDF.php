...
$valorEdicionAprobada=$_POST['edicionSinAprobar'];
...
/// aca verifico si es una edicion despues de haber sido aprobada la preventa o no

//si entra al if es xq no esta aprobada la preventa todavia
if($valorEdicionAprobada=='1'){

	//busco el ultimo id de la preventa para actualizarlo al q guarde en senias_preventa
	$sqlUltPr="SELECT id FROM operacion_preventa ORDER BY id DESC LIMIT 1";
	$resultadoUltp = mysqli_query($conexion,$sqlUltPr);
	 $verultPrId= mysqli_fetch_array($resultadoUltp);


	 $sqlUpdSenia="UPDATE senias_preventa set senias_preventa.id_preventa='$verultPrId[0]'
	  where id_preventa='$ver1[0]' and estado='0'";

	 mysqli_query($conexion,$sqlUpdSenia);



	 //actualizo el registro de la preventa a tiene_senia en 1
	 $sqlUpdTi="UPDATE operacion_preventa set operacion_preventa.tiene_senia='1'
	  where id='$verultPrId[0]'";

	 mysqli_query($conexion,$sqlUpdTi);	
...
$sqlUV ="UPDATE vehiculos SET id_estado = '6' WHERE id = '$idVehiculo'";
 ...
 $sqlParaPdf="SELECT u.nombre as nombreV,u.apellido as apellidoV,cl.nombre as nombreC,cl.apellido as apellidoC,
 v.dominio_patente as patente,v.modelo as modelo ,v.marca as marca, cl.telefono as telefono,v.color as color, 
 o.id_usados_entregados as usadosEntregados, v.precio_venta as precioVenta, v.nombre_vehiculo_infoauto as nomInfo
  from operacion_preventa o,usuarios u,
 clientes cl,vehiculos v where  o.id_vendedor=u.id  and o.id_cliente=cl.id and o.id_vehiculo_prevendido=v.id  
 and o.id_real_preventa='$valor' and o.edicion='$valorEdicion'";
...
$mail->IsSMTP();
$mail->SMTPAuth = true;
$mail->SMTPSecure = "ssl";   // ojo aca aveces va minuscula y aveces mayusculas
...
//  $mail->SMTPDebug = 4;
  ...
  $mail->Port = 465;
  $mail->Username = "juansartor92@gmail.com";
  $mail->Password = "36796806jiSD";

//   $mail->Host = "smtp.hostinger.com.ar";
//    $mail->Port = 587;
//    $mail->Username = "fingino@autogaba.com";
//    $mail->Password = "t=nRB9=7";




//    //  estos deben recibir administracion@autogaba.com... y falta leandro
$mail->From = "juansartor92@gmail.com";
$mail->FromName = "Sistema AutoGaba";
$mail->Subject = "Nueva PreVenta";



	// asunto: poner Nombre vendedor -- Nueva Preventa
	// cuerpo: nro preventa, vendedor, dominio patente,marca modelo tipo, precio venta, fecha que cargan en la preventa
	
	
$mail->MsgHTML("<br>Vendedor:  <b>".$verEnPdf['nombreV'].' '.$verEnPdf['apellidoV']."</b><br><br>
Vehiculo:  <b>".utf8_decode($verEnPdf['marca'].' '.$verEnPdf['modelo'].' '.$verEnPdf['nomInfo'].'  --  '.$verEnPdf['patente'])."</b><br><br><br>
<b>No debe responder este mail.</br>");

	
 $mail->AddAddress("juani_sartor@hotmail.com.ar", "Destinatario");
