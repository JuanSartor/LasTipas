...
$consulta = "SELECT id,numero,estado FROM mesas WHERE id in (" . $ids . ") and eliminado='0'";
$resultadiIds = mysqli_query($conexion, $consulta);

//este array tiene los numeros de mesas en orden
$arrayNumeroMesas = array();
// creo un array para los estados
$arrayEstadosMesas = array();
if ($resultadiIds) {
	if (mysqli_num_rows($resultadiIds) > 0) {
		while ($mostrarids = mysqli_fetch_array($resultadiIds)) {
			foreach ($arregloPrevioAString as &$elemento) {
				...
				if ($mostrarids[0] == $elemento) {
					...
					array_push($arrayEstadosMesas, $mostrarids[2]);
				}
			}
		}
		...
		foreach ($arregloCantidaEleEnPos1 as &$cant) {
			if ($cant > 1) {
				for ($j = 0; $j < $cant; $j++) {
					...
					$estado = $arrayEstadosMesas[$incremento];
					$incremento++;
				}
			...
			$matriz[$incrementoMatriz][3] = $estado;
			$incrementoMatriz++;
			$cadaInsert = '';
		}
		...
		foreach ($matriz as &$elem) {
			$cad = str_replace(",", "000", $elem[2]);
			if($elem[3]=='1'){
			$cadena = $cadena . '<button type="button" onclick="irAComanda(' . $elem[0] . ',' .	 $cad .',1'.')"
				...
				$cadena = $cadena . '<button type="button" onclick="irAComanda(' . $elem[0] . ',' .	 $cad .',2'. ')"
				style="width: 110px; border-radius: 10px; padding: 10px; 
				margin: 20px;" title="Cerrada" class="btn btn-warning mb-1" id="mesaN"' . $elem[2] . '> 
				<span  class="fas fa-tablets"></span> Mesa: ' . $elem[2] . ' </button>';
			}
		}
		echo $cadena;
